...
menu_table = dynamodb.Table('PizzaShop')
...
def postOrder(event,context,item):
    ...
    response = { "Message": "Hi " + customer_name + ", please choose one of these selection: " + ' '.join(['{}. {}'.format(k,v) for k,v in selection.iteritems()])}
...
def putSelection(event,context):
    ...
    if order_status == "selection" :
        ...
        response = { "Message": "Which size do you want? " + ' '.join(['{}. {}'.format(k,v) for k,v in size.iteritems()])}
        ...
        response = { "Message": "Your order costs $" + order['price'] + ". We will email you when the order is ready. Thank you!" }
